...
class AchievementsFacade:
    def get_achievements_payload(self, db: Session, ctx=None):
        return achievement_service.get_achievements_payload(db, ctx=ctx)

    def get_student_achievements_payload(self, db: Session, student_id: int, ctx=None):
        return achievement_service.get_student_achievements_payload(db, student_id, ctx=ctx)

    async def upload_achievement_media(self, db: Session, *, ctx=None, achievement_id: int, file, logger=None):
    ...
    async def upload_achievement_video(self, db: Session, *, ctx=None, achievement_id: int, file):
        return await achievement_service.upload_achievement_video(db, ctx=ctx, achievement_id=achievement_id, file=file)

    async def create_achievement_from_form(self, db: Session, *, ctx=None, title: str, description: str | None, event_date: str | None, place: str | None, assignment_type: str, student_id: int | None, file, logger=None):
    ...
    def assign_achievement(self, db: Session, *, ctx=None, achievement_id: int, user_id: int):
